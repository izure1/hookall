...
> = T extends (initialValue: any, ...params: infer R) => any ? R : never
...
type HookallOnCallback<
  ...
  K extends keyof M
> = (initialValue: Awaited<ReturnType<M[K]>>, ...params: HookallCallbackParams<M[K]>) => Promise<Awaited<ReturnType<M[K]>>>

type HookallTriggerCallback<
  M extends ListenerSignature<M>,
  K extends keyof M
> = (initialValue: Parameters<M[K]>[0], ...params: HookallCallbackParams<M[K]>) => Promise<Awaited<ReturnType<M[K]>>>
...
export interface IHookall<M extends ListenerSignature<M> = DefaultListener> {
  ...
  trigger<K extends keyof M>(command: K & string, initialValue: Awaited<ReturnType<M[K]>>, callback: HookallTriggerCallback<M, K>, ...params: HookallCallbackParams<M[K]>): Promise<Awaited<ReturnType<M[K]>>>
}
...
class Hookall<M extends ListenerSignature<M>> implements IHookall<M> {
  ...
  constructor(target: object) {
    this.beforeHooks = Hookall._Store.ensure(target, 'before') as any
    this.afterHooks = Hookall._Store.ensure(target, 'after') as any
  }
  ...
  private async _hookWith<K extends keyof M>(
    hooks: HookallCallbackMap<M, K>,
    command: HookallLifeCycle<K & string>,
    value: Awaited<ReturnType<M[K]>>,
    ...params: HookallCallbackParams<M[K]>
   ...
   */
  async trigger<K extends keyof M>(
    command: HookallLifeCycle<K & string>,
    initialValue: Awaited<ReturnType<M[K]>>,
